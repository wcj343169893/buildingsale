...
<body onload="selDdict(); er('${error }');">
<form action="validate/~client!selByToscanmodi.action" method="post" onsubmit="return selBysome();">
<input type="hidden" name="operate" value="toScanModiClient"/>
<table width="98%" align="center" class="tab_tab" cellpadding="0" cellspacing="0">
			...
			<table width="100%" border="1" style="border-collapse:collapse;" cellpadding="4" bordercolor="#6BB3ED">
				...
				<c:forEach items="${clients}" var="c">
					<tr class="tab_tr_out" onmouseover="this.className='tab_tr_over'" onmouseout="this.className='tab_tr_out'">
						...
						<td align="center">
							<c:choose>
								<c:when test="${c.dataDict.ddValue=='新客户'}">
								<a href="validate/~client!sToModiclient.action?clientInfo.clientId=${c.clientId }"><img src="images/tab/edit.gif" />更改需求</a>
								</c:when>
								<c:when test="${c.dataDict.ddValue=='已分配'}">
								<a href="validate/~client!sToModiclient.action?clientInfo.clientId=${c.clientId }"><img src="images/tab/edit.gif" />更改需求</a>
								</c:when>
								<c:otherwise>
									<font style="color:#777777;"><img src="images/tab/edit.gif" />更改需求</font>
								</c:otherwise>
							</c:choose>
						</td>
					</tr>
	...
	<tr>
		...
		<td class="three_2" align="right" >
			...
			共 ${allpage }页
			<a href="validate/~client!selToscanmodi.action?page=1" class="page_a">第一页</a>
			<c:choose>
				<c:when test="${allpage==1 }">上一页&nbsp;下一页</c:when>
				<c:when test="${(page-1)<1 }">上一页&nbsp;
						<a href="validate/~client!selToscanmodi.action?page=${page+1 }" class="page_a">下一页</a>
				</c:when>
				<c:when test="${(page+1)>allpage }">
				      <a href="validate/~client!selToscanmodi.action?page=${page-1 }" class="page_a">上一页</a>&nbsp;下一页
				</c:when>
				<c:otherwise>
						<a href="validate/~client!selToscanmodi.action?page=${page-1 }" class="page_a">上一页</a>
						<a href="validate/~client!selToscanmodi.action?page=${page+1 }" class="page_a">下一页</a>
				</c:otherwise>
			</c:choose>
			<a href="validate/~client!selToscanmodi.action?page=${allpage }" class="page_a">末尾页</a>
				...
				<script type="text/javascript">
					function dogopage(pages){
						...
						if(!zz.test(countnum)||!zz.test(pagecou)){
						    ...
						    location.href="~client!selToscanmodi.action?page="+countnum+"&pagecount="+pagecou;
						}
					}
				</script>
