...
<form action="validate/~client!selByToClient.action" method="post" onsubmit="return selBysome();">
<input type="hidden" name="operate" value="toScanAllotClient"/>
<table width="98%" align="center" class="tab_tab" cellpadding="0" cellspacing="0">
			...
			<table width="100%" border="1" style="border-collapse:collapse;" cellpadding="4" bordercolor="#6BB3ED">
				...
				<c:forEach items="${clients }" var="c">
					<tr class="tab_tr_out" onmouseover="this.className='tab_tr_over'" onmouseout="this.className='tab_tr_out'">
						...
						<td align="center">
						  <c:if test="${c.dataDict.ddValue=='新客户'}">
								<a href="validate/~client!toAllotclient.action?clientInfo.clientId=${c.clientId}"><img src="images/tab/edit.gif" />分配</a>
						  </c:if>
						  <c:if test="${c.dataDict.ddValue!='新客户'}">
								<font style="color:#777777;"><img src="images/tab/edit.gif" />分配</font>
						  </c:if>
						</td>
					</tr></c:forEach>
			</table>
	...
	<tr>
		...
		<td class="three_2" align="right" >
			...
			共 ${allpage }页
			<a href="validate/~client!selToClient.action?page=1" class="page_a">第一页</a>
			<c:choose>
				<c:when test="${allpage==1 }">上一页&nbsp;下一页</c:when>
				<c:when test="${(page-1)<1 }">上一页&nbsp;
						<a href="validate/~client!selToClient.action?page=${page+1 }" class="page_a">下一页</a>
				</c:when>
				<c:when test="${(page+1)>allpage }">
				      <a href="validate/~client!selToClient.action?page=${page-1 }" class="page_a">上一页</a>&nbsp;下一页
				</c:when>
				<c:otherwise>
						<a href="validate/~client!selToClient.action?page=${page-1 }" class="page_a">上一页</a>
						<a href="validate/~client!selToClient.action?page=${page+1 }" class="page_a">下一页</a>
				</c:otherwise>
			</c:choose>
			<a href="validate/~client!selToClient.action?page=${allpage }" class="page_a">末尾页</a>
				...
				<script type="text/javascript">
					function dogopage(pages){
						...
						if(!zz.test(countnum)||!zz.test(pagecou)){
						    ...
						    location.href="~client!selToClient.action?page="+countnum+"&pagecount="+pagecou;
						}
					}
				</script>
