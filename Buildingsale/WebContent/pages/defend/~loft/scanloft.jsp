...
<body>
 <form action="validate/~loft!selLoft.action" name="selby" method="post" onsubmit="return doselby();">
...
<table width="98%" align="center" class="tab_tab" cellpadding="0" cellspacing="0">
			...
			<table border="0" cellpadding="0" cellspacing="0" width="100%" height="26">
				<tr>
					...
					<td align="right" style="padding-top:5px;" width="80">
						...
						<input type="button" value="返回" class="tab_but" onclick="doback('~loft!selAllLoft.action');" />
					</td>
				</tr>
			</table>
			...
			<table width="100%" border="1" style="border-collapse:collapse;" cellpadding="4" bordercolor="#6BB3ED">
				...
				<c:forEach items="${lofts}" var="l">
					<tr class="tab_tr_out" onmouseover="this.className='tab_tr_over'" onmouseout="this.className='tab_tr_out'">
						...
						<td align="center">
							<a href="validate/~loft!selById.action?loftinfto.loftId=${l.loftId }"><img src="images/tab/scan.png" />查看</a>&nbsp;&nbsp;
						</td>
					</tr>
				</c:forEach>
			</table>
	...
	<tr>
		...
		<td class="three_2" align="right" >
			...
			共 ${allpage }页
			<a href="validate/~loft!selAllLoft.action?page=1" class="page_a">第一页</a>
			<c:choose>
				<c:when test="${allpage==1 }">上一页&nbsp;下一页</c:when>
				<c:when test="${(page-1)<1 }">上一页&nbsp;
						<a href="validate/~loft!selAllLoft.action?page=${page+1 }" class="page_a">下一页</a>
				</c:when>
				<c:when test="${(page+1)>allpage }">
				      <a href="validate/~loft!selAllLoft.action?page=${page-1 }" class="page_a">上一页</a>&nbsp;下一页
				</c:when>
				<c:otherwise>
						<a href="validate/~loft!selAllLoft.action?page=${page-1 }" class="page_a">上一页</a>
						<a href="validate/~loft!selAllLoft.action?page=${page+1 }" class="page_a">下一页</a>
				</c:otherwise>
			</c:choose>
			<a href="validate/~loft!selAllLoft.action?page=${allpage }" class="page_a">末尾页</a>
				...
				<script type="text/javascript">
					function dogopage(pages){
						...
						if(!zz.test(countnum)||!zz.test(pagecou)){
						    ...
						    location.href="~loft!selAllLoft.action?page="+countnum+"&pagecount="+pagecou;
						}
					}
				</script>
