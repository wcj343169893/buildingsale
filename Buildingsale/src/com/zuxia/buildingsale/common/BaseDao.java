...
public class BaseDao extends HibernateDaoSupport {
	...
	public int getTotalCount(String strhql, List<Object> params) {
		...
		List list = this.getHibernateTemplate().find(
				"select count(*) " + strhql, params.toArray());
		return Integer.valueOf(list.get(0).toString());
	}
	...
	public int getTotalCountBySql(String strsql) {
		List list = this.getSession().createSQLQuery(
				"select count(*) " + strsql).list();
		return Integer.valueOf(list.get(0).toString());
	}
	...
	public void batchDelete(Class clazz, long[] id) {
		for (int i = 0; i < id.length; i++) {
			...
			this.getHibernateTemplate().delete(
					this.getHibernateTemplate().get(clazz, id[i]));
		}
	}
	...
	public List query(Page page, String strhql) {
		...
		if (page != null) {
			// 设置query的第一条数据
			query.setFirstResult(((page.getPageIndex() - 1) * page
					.getPageRecordCount()));
			// 设置query的数据条数
